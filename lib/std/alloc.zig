...
pub const Logging = @import("alloc/logging.zig");
pub const LogToWriter = @import("alloc/log_to_writer.zig");
pub const Arena = @import("alloc/arena.zig");
pub const GeneralPurpose = @import("alloc/general_purpose.zig");
pub const Wasm = @import("alloc/Wasm.zig");
pub const WasmPage = @import("alloc/WasmPage.zig");
pub const Page = @import("alloc/Page.zig");
pub const ThreadSafe = @import("alloc/ThreadSafe.zig");
pub const Sbrk = @import("alloc/sbrk.zig");

pub const MemoryPool = @import("alloc/memory_pool.zig");
...
pub const page_allocator = if (@hasDecl(root, "os") and
    @hasDecl(root.os, "alloc") and
    @hasDecl(root.os.alloc, "page_allocator"))
    root.os.alloc.page_allocator
else if (builtin.target.isWasm())
    Allocator{
        .ptr = undefined,
        .vtable = &WasmPage.vtable,
    }
else if (builtin.target.os.tag == .plan9)
    Allocator{
        .ptr = undefined,
        .vtable = &Sbrk(std.os.plan9.sbrk).vtable,
    }
else
    Allocator{
        .ptr = undefined,
...
pub const FixedBuffer = struct {
    ...
    pub fn allocator(self: *FixedBuffer) Allocator {
        return .{
            .ptr = self,
            .vtable = &.{
                .alloc = alloc,
    ...
    pub fn threadSafeAllocator(self: *FixedBuffer) Allocator {
        return .{
            .ptr = self,
            .vtable = &.{
    ...
    pub fn ownsPtr(self: *FixedBuffer, ptr: [*]u8) bool {
        return sliceContainsPtr(self.buffer, ptr);
    }

    pub fn ownsSlice(self: *FixedBuffer, slice: []u8) bool {
    ...
    pub fn isLastAllocation(self: *FixedBuffer, buf: []u8) bool {
        return buf.ptr + buf.len == self.buffer.ptr + self.end_index;
    }

    fn alloc(ctx: *anyopaque, n: usize, log2_ptr_align: u8, ra: usize) ?[*]u8 {
        const self: *FixedBuffer = @ptrCast(@alignCast(ctx));
        _ = ra;
    ...
    pub fn reset(self: *FixedBuffer) void {
        self.end_index = 0;
    }
};
...
pub fn StackFallbackAllocator(comptime size: usize) type {
    return struct {
        ...
        fixed_buffer_allocator: FixedBuffer,
        ...
        pub fn get(self: *Self) Allocator {
            ...
            self.fixed_buffer_allocator = FixedBuffer.init(self.buffer[0..]);
            return .{
                .ptr = self,
                .vtable = &.{
                    .alloc = alloc,
        ...
        fn alloc(
            ...
            return FixedBuffer.alloc(&self.fixed_buffer_allocator, len, log2_ptr_align, ra) orelse
        ...
        fn resize(
            ...
            if (self.fixed_buffer_allocator.ownsPtr(buf.ptr)) {
                return FixedBuffer.resize(&self.fixed_buffer_allocator, buf, log2_buf_align, new_len, ra);
            } else {
        ...
        fn free(
            ...
            const self: *Self = @ptrCast(@alignCast(ctx));
            if (self.fixed_buffer_allocator.ownsPtr(buf.ptr)) {
                return FixedBuffer.free(&self.fixed_buffer_allocator, buf, log2_buf_align, ra);
            } else {
...
test "Arena" {
    var arena_allocator = Arena.init(page_allocator);
...
test "FixedBufferAllocator" {
    var fixed_buffer_allocator = mem.validationWrap(FixedBuffer.init(test_fixed_buffer_allocator_memory[0..]));
...
test "FixedBufferAllocator.reset" {
    ...
    var fba = FixedBuffer.init(buf[0..]);
...
test "FixedBuffer Reuse memory on realloc" {
    var small_fixed_buffer: [10]u8 = undefined;
    // check if we re-use the memory
    {
        var fixed_buffer_allocator = FixedBuffer.init(small_fixed_buffer[0..]);
        const allocator = fixed_buffer_allocator.allocator();
...
test "Thread safe FixedBuffer" {
    var fixed_buffer_allocator = FixedBuffer.init(test_fixed_buffer_allocator_memory[0..]);
...
pub fn testAllocatorAlignedShrink(base_allocator: mem.Allocator) !void {
    ...
    var fib = FixedBuffer.init(&debug_buffer);
